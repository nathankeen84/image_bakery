...
locals {
  ...
  image_name    = "ubuntu-2204-cis2"
  image_version = var.build_date
}

source "azure-arm" "ubuntu_cis2" {
...
build {
  ...
  provisioner "shell" {
    inline = [
      "echo '=== Starting Ubuntu CIS L2 Build (Incremental on L1) ==='",
      "sudo apt-get update",
  ...
  provisioner "shell" {
    script = "${path.root}/../../../shared/scripts/hardening/cis2/ubuntu-cis2-hardening.sh"
  }
