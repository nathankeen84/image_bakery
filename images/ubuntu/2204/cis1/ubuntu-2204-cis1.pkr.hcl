...
locals {
  ...
  image_name    = "ubuntu-2204-cis1"
  image_version = var.build_date
}

source "azure-arm" "ubuntu_cis1" {
...
build {
  ...
  provisioner "shell" {
    inline = [
      "echo '=== Starting Ubuntu CIS L1 Build ==='",
      "sudo apt-get update",
  ...
  provisioner "shell" {
    script = "${path.root}/../../../shared/scripts/hardening/cis1/ubuntu-cis1-hardening.sh"
  }
