...
locals {
  ...
  image_name    = "azurelinux-3-cis1"
  image_version = var.build_date
}

source "azure-arm" "azurelinux_cis1" {
...
build {
  ...
  provisioner "shell" {
    inline = [
      "echo '=== Starting Azure Linux CIS L1 Build ==='",
      "sudo tdnf update -y",
      "sudo tdnf install -y curl wget git unzip ca-certificates"
    ]
  }

  # Provisioner 2: CIS L1 Hardening (using RHEL script as base for CBL-Mariner/Azure Linux)
  provisioner "shell" {
    script = "${path.root}/../../../shared/scripts/hardening/cis1/rhel-cis1-hardening.sh"
  }

  # Provisioner 3: OS Configuration
  provisioner "shell" {
    inline = [
  ...
  # Provisioner 4: Install Baseline Agents (with secrets from Key Vault)
  provisioner "shell" {
    environment_vars = [
  ...
  # Provisioner 5: Cleanup and finalization
  provisioner "shell" {
    inline = [
