...
locals {
  ...
  image_name    = "windows-2022-cis1"
  image_version = var.build_date
}

source "azure-arm" "windows_cis1" {
...
build {
  ...
  provisioner "powershell" {
    inline = [
      "Write-Host '=== Starting Windows CIS L1 Build ===' -ForegroundColor Green",
  ...
  # Provisioner 2: CIS L1 Hardening
  provisioner "powershell" {
  ...
  provisioner "powershell" {
    inline = [
      ...
      "Write-Host 'Windows image build completed successfully' -ForegroundColor Green"
    ]
  }
}
