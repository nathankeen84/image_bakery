...
locals {
  ...
  image_name    = "windows-2019-cis2"
  image_version = var.build_date
}

source "azure-arm" "windows_2019_cis2" {
...
build {
  ...
  provisioner "powershell" {
    inline = [
      "Write-Host '=== Starting Windows 2019 CIS L2 Build (Incremental on L1) ===' -ForegroundColor Green",
  ...
  provisioner "powershell" {
    script = "${path.root}/../../../cis1/windows-cis1-hardening.ps1"
  }

  provisioner "powershell" {
    script = "${path.root}/../../../cis2/windows-cis2-hardening.ps1"
  }
  ...
  provisioner "powershell" {
    inline = [
      ...
      "Write-Host 'Windows 2019 CIS L2 image build completed successfully' -ForegroundColor Green"
    ]
  }
}
